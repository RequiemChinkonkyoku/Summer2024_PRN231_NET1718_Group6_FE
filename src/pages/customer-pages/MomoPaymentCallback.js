...
const MomoPaymentCallback = () => {
  ...
  React.useEffect(() => {
    if (token) {
      ...
      if (orderInfo) {
        const parts = orderInfo.split("AppointmentID: ");
        const temp = parts.length > 1 ? parts[1].trim() : null;

        console.log(parts);
        console.log(temp);

        // setAppId(temp ? temp.toString() : "");
        axios.defaults.headers.common["Authorization"] = `Bearer ${token}`;
        axios.post(`/Momo/payment-execute/${temp}`).then((response) => {
